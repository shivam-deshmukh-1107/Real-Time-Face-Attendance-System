...
firebase_admin.initialize_app(cred, {
    ...
    'storageBucket': 'faceattendencerealtime-d1917.appspot.com'
})
...
while True:
    ...
    if faceCurrentFrame:
        for encodeFace, faceLock in zip(encodeCurrentFrame, faceCurrentFrame):
            matches = face_recognition.compare_faces(
                encodeListKnown, encodeFace)
            faceDistance = face_recognition.face_distance(
                encodeListKnown, encodeFace)
            # print(matches, faceDistance)
        ...
        if counter != 0:
            if counter == 1:
                ...
                datetimeObject = datetime.strptime(studentInfo['last_attendance_time'],
                                                   "%Y-%m-%d %H:%M:%S")
                sec_elapsed = (datetime.now() - datetimeObject).total_seconds()
                print(sec_elapsed)
                if sec_elapsed > 30:
